...
class RetailAgentixClient:
    """AG-UI Protocol compatible client"""
    ...
    def create_thread(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                ...
                json={"assistant_id": "retail-orchestrator"},
                timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                self.current_thread_id = data.get("id")
                return data
            else:
                return {'error': f"HTTP {response.status_code}: {response.text}"}
        except Exception as e:
            return {'error': f"Failed to create thread: {e}"}
    
    def start_pipeline_sync(self, initial_state=None):
        """Start pipeline with proper AG-UI protocol event handling"""
        ...
        print(f"[Frontend] 🔗 Created thread: {thread_id}")
        ...
        try:
            ...
            if response.status_code == 200:
                result = self.process_sse_stream(response, thread_id)
                
                # ✅ Ensure thread_id and run_id are in the result
                result['thread_id'] = thread_id
                if not result.get('run_id') and self.current_run_id:
                    result['run_id'] = self.current_run_id
                    print(f"[Frontend] 📌 Added run_id to result: {self.current_run_id}")
    ...
    def render_agent_dashboard(self, agent_states: dict):
        """Renders a compact progress dashboard for agent workflow"""
        st.markdown("### 🧠 Agent Workflow Status")
        
        col1, col2, col3, col4, col5 = st.columns(5)
        cols = [col1, col2, col3, col4, col5]
        steps = ["ingestion", "analysis", "prediction", "decision", "execution"]
        icons = {
            "idle": "⚪",
            "running": "🟡",
            "done": "✅",
        ...
        for i, step in enumerate(steps):
            state = agent_states.get(step, "idle")
            with cols[i]:
                st.markdown(f"""
                <div style="text-align:center; padding:8px; border-radius:10px; 
                            background-color:#1E1E1E; color:white;">
                    <div style="font-size:30px;">{icons[state]}</div>
                    <div style="font-size:14px; font-weight:bold;">{step.capitalize()}</div>
                </div>
                """, unsafe_allow_html=True)
    def update_agent_states_from_event(self, agent_states: dict, event_type: str, message: str):
        """Update agent state machine based on AG-UI event messages"""
        ...
        elif "approval" in message_lower:
            agent_states["decision"] = "waiting_approval"
        elif "error" in message_lower:
            for agent in agent_states:
                if agent in message_lower:
                    agent_states[agent] = "error"

        return agent_states

    def process_sse_stream(self, response, thread_id):
        """Process SSE stream with AG-UI protocol events"""
        events = []
        final_state = {}
        status = 'running'
        error_message = None
        ...
        step_order = ["ingestion", "analysis", "prediction", "decision", "execution"]
        agent_states = {s: "idle" for s in step_order}
        dashboard_placeholder = st.empty()
        # Track accumulated state from snapshots
        accumulated_state = {}
        
        # ✅ CRITICAL: Store thread_id immediately in session state
        self.current_thread_id = thread_id
        st.session_state.current_thread_id = thread_id
        print(f"[Frontend] 🔗 Thread ID stored: {thread_id}")
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                if line.startswith('data: '):
                    ...
                    try:
                        ...
                        # ✅ Debug: Log the full event for RUN_STARTED
                        if event_type == 'RUN_STARTED':
                            print(f"[Frontend] 🔍 RUN_STARTED event data: {json.dumps(event_data, indent=2)}")
                        ...
                        if event_type == 'RUN_STARTED':
                            ...
                            print(f"[Frontend] ✅ Run started: {run_id} for thread: {thread_id}")
                            print(f"[Frontend] 📍 Session state - thread_id: {st.session_state.current_thread_id}, run_id: {st.session_state.current_run_id}")
                        
                        elif event_type == 'STATE_SNAPSHOT':
                            # Critical: Extract state from snapshot
                            snapshot = event_data.get('snapshot', {})
                            run_id_from_snapshot = snapshot.get("_run_id")
                            if run_id_from_snapshot:
                                st.session_state.current_run_id = run_id_from_snapshot
                                print(f"[Frontend] Captured run_id from snapshot: {run_id_from_snapshot}")
                            if snapshot:
                                accumulated_state.update(snapshot)
                                print(f"[Frontend] State snapshot received with keys: {list(snapshot.keys())}")
                        
                        elif event_type == 'TEXT_MESSAGE_CHUNK':
                            # Agent progress messages
                            delta = event_data.get('delta', '')
                            if 'approval required' in delta.lower():
                                awaiting_human_approval = True
                                st.session_state.awaiting_approval = True
                                # Extract decisions from accumulated state
                                if 'decision' in accumulated_state:
                                    st.session_state.original_decisions = accumulated_state['decision']
                                
                                # ✅ Ensure IDs are still in session state
                                # if run_id:
                                #     st.session_state.current_run_id = run_id
                                if delta.startswith("[RUN_ID:") and delta.endswith("]\n"):
                                    run_id = delta[len("[RUN_ID:"):-2]
                                    st.session_state.current_run_id = run_id
                                    print(f"[Frontend] Captured run_id from hidden chunk: {run_id}")
                                st.session_state.current_thread_id = thread_id
                                
                                print(f"[Frontend] ⏸️ Human approval required")
                                print(f"[Frontend] 📍 Stored IDs - thread: {st.session_state.current_thread_id}, run: {st.session_state.current_run_id}")
                            self.update_agent_states_from_event(agent_states, event_type, delta)
                            st.session_state.agent_states = agent_states
                            dashboard_placeholder.empty()
                            with dashboard_placeholder.container():
                                self.render_agent_dashboard(agent_states)
                        elif event_type == 'RUN_FINISHED':
                            status = 'completed'
                            # Use accumulated state as final state
                            final_state = accumulated_state
                            st.session_state.agent_states = agent_states
                            print(f"[Frontend] Run finished with state keys: {list(final_state.keys())}")
                            break
                        ...
                        # Check for approval requirement in accumulated state
                        if 'decision' in accumulated_state and not awaiting_human_approval:
                            # Check if we have all previous steps but no execution
                            if ('ingestion' in accumulated_state and 
                                'analysis' in accumulated_state and 
                                'prediction' in accumulated_state and 
                                'execution' not in accumulated_state):
                                awaiting_human_approval = True
                                st.session_state.awaiting_approval = True
                                st.session_state.original_decisions = accumulated_state['decision']
                                ...
                                print(f"[Frontend] 🔍 Detected approval state from snapshot")
                                print(f"[Frontend] 📍 Final check - thread: {st.session_state.current_thread_id}, run: {st.session_state.current_run_id}")
                                break
        ...
        if awaiting_human_approval:
            ...
            if not run_id and self.current_run_id:
                run_id = self.current_run_id
            
            print(f"[Frontend] 📦 Returning awaiting_approval state")
            print(f"[Frontend] 📦 State keys: {list(final_state.keys())}")
            print(f"[Frontend] 📦 Returning with thread_id: {thread_id}, run_id: {run_id}")
    ...
    def approve_decisions(self, thread_id: str, run_id: str, decisions: dict):
        """Send approved decisions to backend"""
        print(f"[Frontend] Sending approval - thread_id: {thread_id}, run_id: {run_id}")
        print(f"[Frontend] Decision keys: {list(decisions.keys())}")
        
        try:
            response = requests.post(
                f"{BACKEND_URL}/threads/{thread_id}/runs/{run_id}/approve",
                json={"decisions": decisions},
                timeout=30
            )
            
            print(f"[Frontend] Approval response status: {response.status_code}")
            
            if response.status_code == 200:
                result = response.json()
                print(f"[Frontend] ✅ Approval successful: {result}")
                return {'status': 'success', 'data': result}
            else:
                error_text = response.text
                print(f"[Frontend] ❌ Approval failed: {response.status_code} - {error_text}")
                return {'error': f"HTTP {response.status_code}: {error_text}"}
        except Exception as e:
            print(f"[Frontend] ❌ Approval exception: {e}")
            import traceback
            traceback.print_exc()
            return {'error': f"Failed to approve decisions: {e}"}
    
    def get_thread_state(self, thread_id: str):
        """Fetch latest thread state from backend"""
        try:
            response = requests.get(
                f"{BACKEND_URL}/threads/{thread_id}/state",
                timeout=10
            )
            if response.status_code == 200:
...
# ---------------- Display helpers (kept mostly unchanged) ----------------
...
def display_decision_data_with_approval(decision_data: Dict[str, Any], analysis_data: Dict[str, Any]):
    """Display decision data with approval interface"""
    ...
    # Show approval banner if awaiting
    ...
    # Display editable or read-only table
    if st.session_state.awaiting_approval:
        ...
        for idx, (item_key, decision_info) in enumerate(decisions.items()):
            with st.expander(f"📦 {item_key.capitalize()} - Edit Decision", expanded=True):
                col1, col2, col3 = st.columns(3)
                
                decision = decision_info.get('decision', {})
                current_price = float(analysis.get(item_key, {}).get('avg_price', 0))
                
                with col1:
                    st.metric("Current Price", f"{st.session_state.currency_symbol}{current_price:.2f}")
                
                with col2:
                    new_price = st.number_input(
                        "New Price",
                        min_value=0.0,
                        value=float(decision.get('suggested_price', current_price)),
                        step=0.01,
                ...
                with col3:
                    ...
                    current_action = decision.get('price_action', 'no_change').replace('_', ' ').title()
                ...
                rationale = decision_info.get('rationale', 'No reasoning provided')
        ...
        with col2:
            if st.button("✅ Submit Approved Decisions", type="primary", use_container_width=True):
                ...
                print("current run id from state",run_id)
                # ✅ Fallback: Try to get from pipeline_result
                if (not thread_id or not run_id) and st.session_state.get('pipeline_result'):
                    pipeline_result = st.session_state['pipeline_result']
                    thread_id = thread_id or pipeline_result.get('thread_id')
                    run_id = run_id or pipeline_result.get('run_id')
                    
                    # Store them back in session state
                    if thread_id:
                        st.session_state.current_thread_id = thread_id
                    if run_id:
                        st.session_state.current_run_id = run_id
                    
                    st.warning(f"⚠️ Retrieved IDs from pipeline_result - thread: {thread_id}, run: {run_id}")
                
                if not thread_id or not run_id:
                    st.error("❌ Missing thread_id or run_id. Please restart the pipeline.")
                    st.error(f"🔍 Debug - thread_id: {thread_id}, run_id: {run_id}")
                    ...
                    with st.expander("🔍 Debug Session State"):
                        st.json({
                            ...
                            "awaiting_approval": st.session_state.get('awaiting_approval'),
                            "pipeline_result_keys": list(st.session_state.get('pipeline_result', {}).keys()) if st.session_state.get('pipeline_result') else []
                        })
                    st.stop()
                
                # Debug info
                st.info(f"🔍 Sending approval for thread: {thread_id}, run: {run_id}")
                ...
                result = client.approve_decisions(
                    thread_id,
                    run_id,
                    approved_decisions['decisions']
                )
                ...
                if 'error' not in result:
                    ...
                    with st.spinner("⚡ Waiting for execution results..."):
                        while elapsed < timeout_seconds:
                            try:
                                thread_resp = client.get_thread_state(thread_id)
                    ...
                    if final_state:
                        ...
                        st.success("⚡ Execution completed! Refreshing dashboard...")
                        st.rerun()
                    else:
                        st.warning("⚠️ Execution taking longer than expected. Please refresh in a moment.")
                else:
                    st.error(f"❌ Failed to approve: {result['error']}")
                    st.error("🔍 Check console logs for details")
...
def main():
    ...
    st.markdown(f"""
    <div style='background: linear-gradient(135deg, {palette_map[st.session_state.palette][0]} 0%, 
                {palette_map[st.session_state.palette][1]} 100%);
    ...
    with st.sidebar:
        ...
        st.markdown("### 🬠Choose Product Category")
        category = st.radio(
            "Select a category:",
            ["🥦 Perishables", "🧁 Bakery"],
            horizontal=True,
        ...
        if st.button(LABELS["btn_launch"], disabled=launch_disabled):
            ...
            if 'error' in result:
                ...
                # ✅ Store IDs from result
                result_thread_id = result.get('thread_id')
                result_run_id = result.get('run_id')
                
                if result_thread_id:
                    st.session_state.current_thread_id = result_thread_id
                if result_run_id:
                    st.session_state.current_run_id = result_run_id
                ...
                # Debug output
                st.info(f"🔗 Thread: {st.session_state.current_thread_id}")
                st.info(f"🆔 Run: {st.session_state.current_run_id}")
            # else:
            #     progress_bar.progress(100)
    ...
    if not result:
        ...
        if not final_state:
            st.warning("📄 Pipeline completed but no state received")
        else:
